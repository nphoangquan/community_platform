...
interface SearchResults { users: User[]; posts: Post[]; }
...
export default function SearchBar() {
  ...
  const debouncedSearch = useCallback((searchQuery: string) => {
    ...
    debounce(async (q: string) => {
      try { const results = await searchContent(q); setSearchResults(results); }
  ...
  const handleViewAllResults = () => {
    if (query.trim().length >= 2) { router.push(`/search/results?q=${encodeURIComponent(query)}`); setIsSearchOpen(false); }
  };

  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => { if (searchRef.current && !searchRef.current.contains(event.target as Node)) setIsSearchOpen(false); };
    document.addEventListener("mousedown", handleClickOutside);
    return () => { document.removeEventListener("mousedown", handleClickOutside); };
  }, []);

  useEffect(() => {
    const handleEscKey = (event: KeyboardEvent) => { if (event.key === "Escape") { setIsSearchOpen(false); setQuery(""); } };
            ...
            <>
                ...
                <div className="p-2">
                  ...
                  <ul>
                    {searchResults.users.map((user) => (
                      <li key={user.id}>
                        <Link href={`/profile/${user.username}`} className="flex items-center gap-3 p-2 hover:bg-zinc-700/50 rounded-md" onClick={() => { setIsSearchOpen(false); setQuery(""); }}>
                          <div className="relative w-8 h-8 overflow-hidden rounded-full bg-zinc-700">
                            {user.avatar && (<Image src={user.avatar} alt={user.username} fill className="object-cover" />)}
                          </div>
                          <div>
                            <p className="text-zinc-200 text-sm font-medium">{user.name} {user.surname}</p>
                ...
                <div className="p-2">
                  ...
                  <ul>
                    {searchResults.posts.map((post) => (
                      <li key={post.id}>
                        <Link href={`/post/${post.id}`} className="flex items-center gap-3 p-2 hover:bg-zinc-700/50 rounded-md" onClick={() => { setIsSearchOpen(false); setQuery(""); }}>
                          <div className="relative w-8 h-8 overflow-hidden rounded-full bg-zinc-700">
                            {post.user.avatar && (<Image src={post.user.avatar} alt={post.user.username} fill className="object-cover" />)}
                          </div>
                          <div className="flex-1 overflow-hidden">
                            <p className="text-zinc-400 text-xs">{post.user.name || post.user.username}</p>
                            <p className="text-zinc-200 text-sm font-medium truncate">{post.desc.length > 50 ? post.desc.substring(0, 50) + "..." : post.desc}</p>
                          </div>
                        </Link>
                      </li>
                    ))}
              ...
              {shouldShowViewAll && (
                <div className="p-2 border-t border-zinc-700">
                  <button onClick={handleViewAllResults} className="w-full p-2 flex items-center justify-center gap-2 bg-zinc-700/50 hover:bg-zinc-700 rounded-md text-zinc-300 hover:text-zinc-100 transition-colors">
